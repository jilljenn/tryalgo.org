...
On peut aussi énumérer en modifiant cette méthode tous les ordres d'évaluations possibles sur le graphe. Pour cela, on énumère tous les ordres possibles sur l'ensemble des noeuds sans dépendances, et, pour chaque ordre, on lance la procédure décrite ci-dessus, en ne choisissant plus les noeuds sans dépendances arbitrairement, mais selon l'ordre que l'on s'est fixé préalablement (si un noeud est placé devant un autre dans un ordre, on sélectionnera ce noeud en premier lors de l'étape de choix de noeuds sans dépendances).

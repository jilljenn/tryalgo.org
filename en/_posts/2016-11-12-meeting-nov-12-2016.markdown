...
* Une approche possible donc. Initialement C est une liste vide qui contiendra les longueurs de cycles pairs.  Et `visited` un tableau boolean encore tout faux. Puis:
  ...
  1. Pour toute lettre pas encore visité. Marquer visite et passer à la lettre suivante comme spécifiée par la permutation. Jusqu'à ce qu'on rencontre un sommet déjà visité. Alors ajouter à C la longueur du cycle.
...
J'ai cherché un peu vite fait. Je ne trouve pas grand chose. On peut déjà se ramener au cas où on a des 'X' solitaires espacés d'au moins 5 '.', puisqu'en dessous, personne ne voudra jouer puisque c'est une position perdante.
Ensuite, mis à part les cas aux bords, on se retrouve à étudier les configurations locales de la forme 'X' + k * '.' + 'X', le jeu total étant le produit de ces configurations locales. Je ne suis pas allé plus loin.
